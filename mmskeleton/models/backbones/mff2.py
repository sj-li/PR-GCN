...
class MFFNet(nn.Module):
    ...
    def __init__(self,
        ...
        if edge_importance_weighting:
            self.edge_importance_fusion = nn.ParameterList([
                nn.Parameter(torch.ones(self.A.size()))
                for i in self.fusion_layers
            ])
            self.edge_importance = nn.ParameterList([
                nn.Parameter(torch.ones(self.A.size()))
                for i in self.posses_layers
            ])
        else:
            self.edge_importance_fusion = [1] * len(self.fusion_layers)
            self.edge_importance= [1] * len(self.posses_layers)
...
class st_gcn_block(nn.Module):
    ...
    def __init__(self,
        ...
        padding = ((dilation*(kernel_size[0]-1))//2, 0)
